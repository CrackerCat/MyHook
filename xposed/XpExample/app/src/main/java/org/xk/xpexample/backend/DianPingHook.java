...
class DianPingHook
{
    public void HookNetMoudle(XC_LoadPackage.LoadPackageParam lpparam)
    {
        ...
        Class<?> ClientCall = XposedHelpers.findClass("com.sankuai.meituan.retrofit2.ClientCall", lpparam.classLoader);
        Class<?> Request = XposedHelpers.findClass("com.sankuai.meituan.retrofit2.Request", lpparam.classLoader);
        XposedHelpers.findAndHookMethod(ClientCall, "getResponseWithInterceptorChain", Request, new XC_MethodHook()
        {
            @Override
            protected void afterHookedMethod(MethodHookParam param) throws Throwable
            {
                ...
                // com.sankuai.meituan.retrofit2.Request
                String url = (String) Reflect.on(request).call("url").get();
                String method = (String) Reflect.on(request).call("method").get();
                Object requestBody = Reflect.on(request).call("body").get();
                XposedBridge.log("url:" + method + " " + url);
                XposedBridge.log("body:" + requestBody.toString());

                if(requestBody != null)
                {
                    String contentType = Reflect.on(requestBody).call("contentType").get();
                    if(contentType != null && contentType.equals("application/x-wwww-form-urlencoded"))
                    {
                        ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
                        Reflect.on(requestBody).call("writeTo", outputStream);
                        XposedBridge.log("");
                        XposedBridge.log("request:" + contentType + " => " + outputStream.toString());
                        outputStream.close();
                    }
                }
            }
